...
- `overflow-hidden` retiré de la carte de famille (`SkillBadges.tsx`). `min-w-0` + `break-words` (remplace `[overflow-wrap:anywhere]`, la patch confirme que `break-words` suffit) restent seuls responsables du repli d'un libellé long — vérifié : le badge « Conception de workflows humain ↔ LLM ↔ agent de code » (le plus long) reste bien dans sa carte.
- Vérification demandée par le patch sur le `transform` de reveal : `getComputedStyle` après la fin de l'animation (page entièrement défilée) donne `transform: none` sur chaque carte, pas de résidu `translateY(0px)` — motion nettoie bien le style en fin d'animation `once: true`. Le contexte d'empilement redouté n'existe donc pas dans ce cas précis.
- Le portail a quand même été choisi comme correctif — pas seulement en filet de secours — plutôt qu'une remontée de `z-index` sur la carte survolée : `SkillBadges.tsx` refactorisé (nouveau sous-composant `SkillBadgeItem`, un `ref`/`useState` par badge, sur le modèle déjà établi par `EvidenceBadge.tsx`) pour utiliser la primitive existante `PortalTooltip` (fix `f856c54`, déjà utilisée dans le Simulateur et le Playground) : la bulle est rendue directement dans `document.body`, `position: fixed`, `z-50`, calculée depuis `getBoundingClientRect()` du badge, et se bascule au-dessus automatiquement quand la place manque en dessous — exactement le comportement demandé au point 3 du patch, déjà écrit et testé ailleurs dans le code, sans dupliquer une primitive.
- **Validation du critère obligatoire** (repli CDP headless, extension Claude in Chrome non connectée cette session) : survol/tap du **dernier badge de chaque famille** (les 6, y compris RAG qui est maintenant le dernier de « No-code / automatisation ») à **1280 px** et à **375 × 600 px** (badge scrollé au ras du bas de l'écran, le pire cas) → bulle trouvée, texte complet, `position: fixed`, `opacity: 1`, dans le viewport, et topmost au point central (`elementFromPoint` renvoie bien un élément de la bulle) pour les 6×2 = 12 combinaisons. À 375×600, la bulle bascule effectivement au-dessus du badge (comportement observé, pas seulement supposé).

**§2 — RAG déplacée vers « No-code / automatisation »** (`src/content/skills.ts`) : niveau et note inchangés. Effet de bord radar recalculé sur l'état réel de `skills.ts` (le patch citait un calcul basé sur une version antérieure au patch skills-7niveaux, déjà signalée obsolète une fois) : axe « LLMs & agents » 3,5 → 4,0 ; axe « No-code / automatisation » **descend** de 3,0 à 2,5 (le patch affirmait à tort qu'il « restait à 1,0 » — cette famille a déjà « Conception de workflows » à 5 depuis un patch précédent). Corrigé dans `specs/06_skills-timeline.md`, signalé plutôt que recopié tel quel.

**§3 — Rééquilibrage, les deux changements validés par Xav en session** (aucune trace écrite du choix n'existait dans le repo ni dans le patch — demandé explicitement avant d'exécuter) : Godot/GDScript 2→3 (haTD prototype jouable playtesté + RPG en développement = définition du 3), Vulgarisation/formation 3→4 (cohérence avec l'offre « Formation équipe (4D) » du site). Effet radar (non demandé au report, noté ici pour mémoire, chiffres du patch également erronés pour ce point — recalculés) : Développement reste affiché à 3,0 (3,2 arrondi, pas 3,5) ; Humain passe de 4,0 à 4,5 (le patch partait à tort d'une base à 3,0, alors que Langues est chiffrée à 5 depuis le patch 9b).

**Vérifié** : `npm run test` (91 tests), `lint`, `build` verts. Version 0.9.4 (et non 0.6.3 comme demandé — même écart de version déjà signalé sur le patch précédent, cette fois clairement récurrent : troisième patch de suite à demander une version `0.6.x` alors que le document réel est en `0.9.x`).

**[ARRÊT XAV]** posé pour la mesure réelle sur Galaxy A04 / Firefox Android (`?perf=lite` puis `?perf=full`) : hors de portée de cette session (pas d'accès à un terminal Android réel), demandée explicitement par le patch et par Xav en session.

## 2026-09-17 — Correctif Z : Compétences invisible sur mobile + débordement badge Cadre 4D (`PATCHES_2026-09-17_fix-skills-mobile.md`)

**Primitive de reveal retrouvée avant modification** : `src/components/ui/SectionShell.tsx`, ligne 28 — `viewport={{ once: true, amount: 0.2 }}` posé sur le `m.div` englobant l'en-tête *et* `children` de **toutes** les sections (un seul déclencheur `whileInView`, propagé aux deux blocs enfants via `variants`). Valeur notée ici avant tout changement, comme demandé.

**Bug 1 — Compétences rendue mais invisible pour toujours sur Firefox Android**. Cause confirmée : `amount: 0.2` exige que 20 % de la hauteur totale du conteneur soit visible pour déclencher `whileInView` ; pour un conteneur plus haut qu'un écran de téléphone, ce ratio (`hauteur du viewport / hauteur du conteneur`) ne peut jamais atteindre 20 % — mesuré à 3677 px de haut à 375 px de large pour Compétences (avec 7 niveaux + bandeau Positionnement) contre 600 px de viewport, soit ~16 %, sous le seuil. `opacity: 0` restait donc posé indéfiniment. Corrigé en deux volets, les deux nécessaires (vérifié par calcul, une tenue à 375×600 pourrait rester sous le seuil même à 0,1 pour un contenu encore plus haut) :
- **Primitive partagée** (`lib/motion.ts` + `components/ui/Reveal.tsx`, nouveau) : seuil abaissé à `amount: 0.1` avec `margin: "0px 0px -10% 0px"`, modifié une seule fois à la source (pas de variante dupliquée pour Compétences). Règle ajoutée en commentaire (`lib/motion.ts`, `SectionShell.tsx`) et dans `specs/00_ROADMAP.md` § Conventions : jamais `amount` > 0,2 sur un conteneur qui peut dépasser un écran de téléphone.
- **Reveal déplacé du niveau section au niveau bloc pour Compétences** : `SectionShell` reçoit un nouveau prop `revealContent` (défaut `true`, inchangé pour toutes les autres sections) ; `Skills.tsx` passe `revealContent={false}` et le reveal se pose désormais sur le radar, sur la légende + micro-ligne, sur le bandeau Positionnement (`SkillRadar.tsx`) et sur chaque carte de famille (`SkillBadges.tsx`) séparément — jamais sur la section entière.
- **Filet de sécurité** (nouveau, absent avant ce patch) : `Reveal` saute l'état caché — contenu visible d'emblée, sans transition — sous `prefers-reduced-motion` ou si `IntersectionObserver` est absent (`shouldSkipReveal`, `lib/motion.ts`). Non lié au mode `lite`. Vérifié à l'écran (repli CDP headless) : sous reduced-motion, `opacity` du bloc d'en-tête = `1` dès le chargement, sans scroll — alors qu'en mode normal le même bloc est correctement à `opacity: 0` tant qu'il n'a pas été scrollé en vue (reveal toujours actif, pas cassé par le correctif).
- **Autres sections vérifiées au test 600 px** (Hero, Simulateur, Playground, Projets, Contact, Jouer) : toutes passent après un défilement complet de la page à 375×600 (`opacity: 1` sur chaque `<section>` en fin de parcours) — aucune n'a eu besoin du traitement bloc par bloc, le seuil à 0,1 suffit pour leur hauteur actuelle. La FAQ n'est pas un `<section>` scrollable (pop-up `Modal` ouvert depuis Jouer, mécanisme `scaleIn`/`scaleInReduced` indépendant) : non concernée par ce bug.

**Bug 2 — Badge Cadre 4D débordait de la carte Méthode IA à 375 px**. Deux volets appliqués, les deux :
- Contenu (`src/content/skills.ts`, `specs/06_skills-timeline.md` §2) : `name` raccourci à `Cadre 4D`, développé des 4D déplacé dans `note` (« Délégation · Description · Discernement · Diligence — pratique documentée »), visible au survol/tap comme les autres notes.
- CSS (`SkillBadges.tsx`) : `min-w-0` sur le conteneur de badge, `[overflow-wrap:anywhere]` sur le texte du bouton, `overflow-hidden` sur la carte de famille (devenue un `Reveal`). Vérifié à l'écran à 375 px : plus aucun débordement, la note s'affiche bien au tap.

**Vérifié à l'écran** (`vite preview` + repli CDP headless, extension Claude in Chrome non connectée cette session) — **au viewport fixe 375 × 600 px, celui qui reproduit le bug, pas 375 × 812** : les 7 sections s'affichent sans zone vide après un parcours de scroll complet ; Compétences visible dès l'entrée dans le viewport (radar, légende, bandeau Positionnement, cartes de famille) ; badge Cadre 4D dans sa carte, tap → tooltip avec les 4D. Rendu desktop 1280 px : pixel-identique à avant le patch hors la ligne Cadre 4D, qui est désormais plus courte (hauteur totale de la section 2838→2796 px, cohérent avec un badge plus court). `npm run test` (91 tests), `lint`, `build` verts. **Mesure Galaxy A04 / Firefox Android réelle non faite cette session** (limite outil : pas d'accès à un vrai terminal Android) — **[ARRÊT XAV]** posé sur le critère §2 de la spec (Galaxy A04, `?perf=lite` puis `?perf=full`), conformément à la demande explicite de s'arrêter à ce point.

**Écart signalé plutôt qu'exécuté tel quel** : le patch demandait la version `0.6.2`, mais `specs/00_ROADMAP.md` était déjà à `0.9.2` (dont un `v0.6.1` similaire déjà demandé — et ignoré à raison — par le patch skills-7-niveaux précédent). Numéro de version incohérent avec l'historique réel du document ; appliqué **0.9.3** à la place (convention `X.Y.Z`, Z = correctif de l'agent), pour ne pas faire reculer le numéro de version. Signalé à Xav plutôt que recopié tel quel.
