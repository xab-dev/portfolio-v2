...
### Projets V1 (7) — contenu réel, à enrichir **[DETTE-10]**
...
3. **miniCiel / Kit USB d'intervention IT** — Outils, Automations · v1 fonctionnelle · 2026 · émeraude
   - Problème : diagnostiquer et entretenir un PC client en 30 min, sans installation, sans réseau, avec compte-rendu.
   - Architecture : dashboard modulaire (découverte auto des modules via `module.json`), scripts PowerShell/Python, aucune collecte de données ; IA utilisée pour concevoir/coder, absente à l'exécution.
   - Métriques : "Système de modules auto-découverts" (verified) ; "Temps de diagnostic cible : 0-30 min" (hypothèse).
   - Limites : pas encore testé chez un client réel (14/09/2026) ; clé bootable non réalisée.
...
5. **Régie Maison** — Automations, Outils · v1 fonctionnelle · [année ? DETTE-12] · émeraude
...
6. **SafeFolder Perso** — Outils · en développement · 2026 · bleu
   - Problème : réorganiser 54 Go de fichiers accumulés sans jamais rien perdre.
   - Architecture : CLI Python, 3 primitives seulement, plan figé par SHA-256, journal avant exécution, annulation par plan inverse, test AST anti-écriture ; Claude Code piloté par `CLAUDE.md` + étapes avec points d'arrêt humains.
   - Métriques : "0 suppression possible par construction" (verified par le design) ; "Étape atteinte : N/M" [DETTE-13].

7. **Un Autre Monde (1AM)** — Formations · cadrage · 2026 · violet
...
3. Ouvrir/fermer une modale au clavier, deep link `#projets/safefolder` fonctionne au rechargement.
4. Tilt inactif sur écran tactile (émulation DevTools) et en reduced-motion.
5. **[ARRÊT XAV]** : relecture des 7 fiches avant de clore la phase.

## 8. Hors scope
Images/captures par projet (DETTE-14), études de cas longues, témoignages clients.
