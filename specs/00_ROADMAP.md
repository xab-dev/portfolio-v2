...
- **Phase 9c — Thème clair / sombre** (`12_theme-clair-sombre.md`) : ouverte sur un constat d'usage — une lectrice a dû abandonner la lecture à cause du blanc sur noir. Thème clair (fond blanc, texte noir) et bouton de bascule au milieu du bandeau, `localStorage` clé `theme`, défaut = `prefers-color-scheme` suivi à chaud. **Exception explicite et bornée à la règle « Design et animations : figés »** ci-dessous : le rendu **sombre** reste la référence au pixel près (D4 de la spec 12), on lui ajoute un rendu clair — vérifié par un diff pixel automatisé (48 captures, 0 px) et un audit de contraste axe-core en clair (0 violation). **Exécutée le 2026-09-22**, deux défauts trouvés et corrigés en route : le mode allégé ne gagnait plus en sombre (spécificité, pas ordre — voir `process/JOURNAL_DEV.md`) et les modales étaient illisibles en clair. DETTE-40 (`--border-glass`) et DETTE-41 (gouttière du panneau de prompt en sombre) ouvertes. **[ARRÊT XAV]** : vérification PC, push, Galaxy A04, relecture de la version claire par sa mère.
