...
**Version : 0.10.2** — document vivant. Créé le 2026-09-15 à partir de la dictée de Xav (12:00) ; révisé le 2026-09-15 12:30 (S1-S7) (dettes de contenu — voir `process/dette_suivi.md` §B), le 2026-09-15 12:45 (D4 tranché, DETTE-25 tranché, scope Phase 6 étendu), le 2026-09-15 (soir) (Phase 6 close et poussée, Phase 6b ajoutée — spec 08, DETTE-30), le 2026-09-15 (nuit) (Phase 6b close — mesure terrain Galaxy A04 validée par Xav), le 2026-09-15 19:30 (Phase 2 close, remise sur `main`, DETTE-09 tranchée — `PATCHES_2026-09-15_1930.md`), le 2026-09-15 23:00 (patch de relecture + phases post-V1 tranchées par Xav — `PATCHES_2026-09-15_2300.md`), le 2026-09-16 (spec 09 Phase 7 écrite, décisions Xav du 16/09 ; Phase 7 exécutée et committée, `7b3821f`), le 2026-09-16 (Phase 8 exécutée depuis `specs/09_export-cv-pdf.md`, D6 posée, DETTE-35/36 ouvertes — DETTE-32/33 déjà pris par la Phase 7, numérotation corrigée par l'agent), le 2026-09-16 (Phase 9 exécutée depuis `specs/10_faq-comment-ce-site.md`, ARRÊT XAV posé §7.8, non close), puis le 2026-09-17 01:00 (relecture 9b par Xav → Phase 9 close, patch `PATCHES_2026-09-17_0100.md`, D7 tranchée), puis le 2026-09-17 (échelle de compétences 1→7 + bandeau Positionnement, spec 06 révisée — correctif de contenu, pas de nouvelle phase), puis le 2026-09-17 (correctif Z, `PATCHES_2026-09-17_fix-skills-mobile.md` : section Compétences invisible sur mobile + débordement du badge Cadre 4D), puis le 2026-09-17 (correctif Z, `PATCHES_2026-09-17_skills-tooltip-rag.md` : bulles de badge coupées par la carte, RAG déplacée vers No-code / automatisation, Godot 2→3 et Vulgarisation 3→4 validés par Xav). Révisé le 2026-09-22 (**0.9.4 → 0.10.0**) : Phase 9c — thème clair / sombre (`12_theme-clair-sombre.md`) exécutée. Nouvelle phase complète, d'où l'incrément de Y. Puis révisé le 2026-09-22 (**0.10.0 → 0.10.1**) : Phase 9c/4 — avatar par thème (deux tirages du même dessin, le sombre inchangé au pixel). Correctif dans le périmètre de la spec 12, pas une nouvelle phase, d'où l'incrément de Z. A révélé et corrigé un défaut de la Phase 9c : `useTheme` sans état partagé (voir `process/JOURNAL_DEV.md`). DETTE-42 ouverte. Puis révisé le 2026-09-22 (**0.10.1 → 0.10.2**) : ménage de dépôt hors phase — documents personnels sortis du suivi Git, `JOURNAL_DEV.md`/`dette_suivi.md` déplacés dans `process/`, `README.md` de vitrine écrit depuis le texte de Xav, version `package.json` réalignée (0.7.0 → 0.10.1 → 0.10.2). Aucun code applicatif touché, d'où l'incrément de Z. DETTE-43 ouverte.
...
- **Repo GitHub neuf** `xab-dev/portfolio-v2` (https://github.com/xab-dev/portfolio-v2), publié via GitHub Pages à `https://xab-dev.github.io/portfolio-v2/`. Il contient déjà `specs/` et `process/dette_suivi.md`, rien d'autre. Ce site est le **portfolio officiel** de Xav.
...
- Les données de contenu (projets, compétences, parcours, réponses de l'agent) sont fournies dans les specs 02 à 07. Tout ce qui est marqué `[À CONFIRMER]` ou `[DETTE-xx]` est référencé dans `process/dette_suivi.md` : ne l'invente pas, mets une valeur neutre et laisse le marqueur.
...
**Reveal au scroll (`Reveal`, `src/components/ui/Reveal.tsx`, règle posée par `PATCHES_2026-09-17_fix-skills-mobile.md`, Bug 1)** : jamais de `amount` > 0,2 sur un conteneur dont la hauteur peut dépasser un écran de téléphone — au-delà, la fraction de visibilité exigée n'est jamais atteinte et le contenu reste caché pour toujours (vu en clair sur Galaxy A04 / Firefox Android, section Compétences). Le reveal se pose sur des blocs d'un écran maximum (carte, groupe de badges), jamais sur une section entière.
...
3. **Scope discipline.** Une phase = un fichier de spec. Un problème découvert hors phase est **documenté dans `process/JOURNAL_DEV.md`**, pas corrigé en silence — sauf s'il bloque la phase courante.
4. **Pas de placeholders vides** (`// TODO ici`) dans le code livré. Si une donnée manque, elle est représentée par une valeur neutre explicite + marqueur `[DETTE-xx]` référencé dans `process/dette_suivi.md`.
5. **Langue** : UI, commentaires, commits, docs en français. Code en anglais.
6. **Fichiers protégés** : `process/dette_suivi.md` (l'agent y *ajoute* des lignes, ne supprime jamais). Le repo `cv-portfolio` (Carnet de terrain + haTD) n'est jamais modifié depuis ce projet — le jeu y est seulement *lu* par URL.
7. **Fin de session** : (1) décisions prises et pourquoi, (2) livré et validé, (3) hors scope et pour quelle phase — dans `process/JOURNAL_DEV.md`.

---

## Phase 0 — Socle Vite + design system + squelette navigable
**Livrée le 2026-09-15.** Détail complet : `01_socle-design-system.md`. **Phase à exécuter maintenant : Phase 8 (`09_export-cv-pdf.md`)**, exécutée cette session — ARRÊT XAV posé, non close.
...
- `process/JOURNAL_DEV.md` et `process/dette_suivi.md` copiés dans le repo.
...
- **Phase 7 — Polish, perf, SEO, mentions légales** (`09_polish-seo-mentions-legales.md`) : page Mentions légales complète (identité, SIRET, hébergeur GitHub, RGPD du formulaire — obligation légale pour un site professionnel, à ne pas repousser), SEO/OG/sitemap/JSON-LD Person sur l'URL actuelle `xab-dev.github.io/portfolio-v2/` (domaine personnalisé : **statu quo**, décision de Xav, à réévaluer avec les finances), vérification et liens des 4 références (DETTE-31), largeur des panneaux du Playground (longues lignes), audit `prefers-reduced-motion` avec un vrai outil d'émulation, Lighthouse à reconfirmer. **L'image OG définitive attend le « skin » perso** (voir post-V1) : en Phase 7, OG = avatar + titre, sobre. Décisions du 16/09 : domicile complet publié, TVA art. 293 B, vue `#mentions-legales` pleine page, JSON-LD complet (mail + téléphone inclus), OG générée par script sharp, `noindex` jusqu'au polish final, 4ᵉ référence retirée / `saisie` non chiffrée, liens de références dans le badge, Playground en retour à la ligne, audit reduced-motion par émulation seule, DETTE-32 traitée à la main par Xav.
- **Phase 8 — Export CV classique (PDF une page)** (`09_export-cv-pdf.md`) : généré depuis `src/content/*.ts` (projets, skills, timeline, contact) pour ne jamais diverger du site ; bouton « Télécharger le CV » dans le Hero et le Contact ; génération à la build (`npm run cv`, `prebuild`/`predev`, fichier statique dans `dist/cv/`) plutôt qu'à la volée côté client. **Livrée et close le 2026-09-16** : `buildCvModel.ts` pur + testé (14 tests), rendu `@react-pdf/renderer`, contrôle 1 page (`pdf-lib`) qui fait échouer le build en cas de débordement. DETTE-35/36 ouvertes (DETTE-32/33 déjà pris par la Phase 7 — numérotation corrigée). PDF relu et validé par Xav, D6 tranchée (tel quel, aucune expérience avant 2026).
- **Phase 9 — FAQ « Comment ce site a été créé ? »** (`10_faq-comment-ce-site.md`) : bouton en haut à droite de la section Jouer, **absent de la nav et du flux principal** ; ouvre un pop-up (primitive `Modal`) contenant une **discussion factice** au format de l'agent de poche (`AgentMessage`/`TypingText`) qui répond à la question principale — le site lui-même comme cas réel : specs numérotées, Claude Code, phases, dette suivie, vérification visuelle, mode allégé après test terrain. Contenu dans `src/content/faq.ts`, dérivé de `process/JOURNAL_DEV.md` (rien d'inventé). 6 questions : workflow, création du site (prioritaires), spec, phases, dette, arrêt humain ; deep link `#faq` ; prop `headerAction` ajoutée à `SectionShell` pour Jouer uniquement. **Exécutée le 2026-09-16, poussée sur `main` sur autorisation de Xav ; ARRÊT XAV §7.8 (relecture des réponses à l'écran, PC + Galaxy A04 en lite) explicitement reporté par Xav à avant la Phase 10 — phase non close.**
- **Phase 9c — Thème clair / sombre** (`12_theme-clair-sombre.md`) : ouverte sur un constat d'usage — une lectrice a dû abandonner la lecture à cause du blanc sur noir. Thème clair (fond blanc, texte noir) et bouton de bascule au milieu du bandeau, `localStorage` clé `theme`, défaut = `prefers-color-scheme` suivi à chaud. **Exception explicite et bornée à la règle « Design et animations : figés »** ci-dessous : le rendu **sombre** reste la référence au pixel près (D4 de la spec 12), on lui ajoute un rendu clair — vérifié par un diff pixel automatisé (48 captures, 0 px) et un audit de contraste axe-core en clair (0 violation). **Exécutée le 2026-09-22**, deux défauts trouvés et corrigés en route : le mode allégé ne gagnait plus en sombre (spécificité, pas ordre — voir `process/JOURNAL_DEV.md`) et les modales étaient illisibles en clair. DETTE-40 (`--border-glass`) et DETTE-41 (gouttière du panneau de prompt en sombre) ouvertes. **[ARRÊT XAV]** : vérification PC, push, Galaxy A04, relecture de la version claire par sa mère. *État au 2026-09-22 (fin de session) : PC (Firefox), push et Galaxy A04 vérifiés par Xav (plus un iPhone émulé F12) — reste dûe la seule relecture par un tiers.*
- **Note de nommage** : les fichiers `09_polish-seo-mentions-legales.md` et `09_export-cv-pdf.md` portent le même numéro (collision constatée à l'écriture de la spec 10). La FAQ prend le **10** et la version anglaise prendra le **11** ; les deux `09` ne sont pas renommés (liens dans le journal).
...
Ordre post-V1 (imposé par Xav, patch 23:00) : patch 23:00 → 7 → 8 → 9 → **9c** → 10 — chaque phase qui ajoute du contenu (CV, FAQ) précède la traduction. La Phase 9c (thème clair/sombre) s'intercale après la 9 ; **la prochaine phase reste la 10 (version anglaise, spec 11)**. **Design et animations : figés**, on n'y touche plus (validés par Xav) — *seule exception à ce jour, bornée et écrite : la Phase 9c, qui ajoute un thème clair sans toucher au rendu sombre.*

## Rappel pour l'agent en fin de session
Avant de conclure : documenter explicitement (1) les décisions prises et pourquoi, (2) ce qui est livré et validé *à l'écran*, (3) ce qui reste hors scope et pour quelle phase c'est prévu, (4) toute nouvelle ligne ajoutée à `process/dette_suivi.md`.
