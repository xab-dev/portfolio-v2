...
| ☐ | **[DETTE-26]** Lighthouse mobile Performance = 86 en local (`vite preview`, throttling simulé standard) au lieu des 90 requis par le critère de passage Phase 0 — TBT et CLS parfaits (0), score 100 sans throttling (FCP 0.1s/LCP 0.9s) : écart structurel lié au rendu 100% client (CSR, pas de SSR/SSG par choix T1), pas un bug de code. Déjà optimisé dans cette session (LazyMotion + chunk d'animations différé, sous-ensembles de polices réduits : JS principal 400 Ko → 328 Ko / 127 Ko → 106 Ko gzip). Reste à mesurer sur le vrai déploiement GitHub Pages (CDN) et à traiter en profondeur (prérendu du shell, etc.) en Phase 7. | Phase 7 |
